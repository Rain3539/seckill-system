...
配置两个 HikariCP 连接池，分别连接主库和从库：

```java
@Bean("masterDataSource")
public DataSource masterDataSource() {
    HikariDataSource ds = new HikariDataSource();
    ds.setJdbcUrl(masterUrl);      // jdbc:mysql://mysql-master:3306/seckill_db
    ds.setUsername("seckill");      // 主库用户（读写权限）
...
public DataSource slaveDataSource() {
    HikariDataSource ds = new HikariDataSource();
    ds.setJdbcUrl(slaveUrl);       // jdbc:mysql://mysql-slave:3306/seckill_db
    ds.setUsername("seckill_ro");   // 从库用户（只读权限）
...
项目提供了 `RwSplitTestController` 用于验证读写分离效果：

| 接口 | 功能 |
|------|------|
| `GET /api/rw/check` | 检查主从数据源连接状态 |
| `GET /api/rw/master-info` | 获取主库信息（server-id等） |
| `GET /api/rw/slave-info` | 获取从库信息（server-id等） |
| `POST /api/rw/route-master` | 手动路由到主库测试 |
| `POST /api/rw/route-slave` | 手动路由到从库测试 |
