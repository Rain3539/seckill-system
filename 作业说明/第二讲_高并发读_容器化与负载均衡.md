...
系统通过 `docker-compose.yml` 编排了 11 个服务容器：
![Docker Compose 网络拓扑](../说明图片/第二讲-docker.svg)

### 2.2 后端 Dockerfile（多阶段构建）

两个微服务（stock-service、order-service）采用相同的**多阶段构建**模式优化镜像体积：
...
order-service 的 Dockerfile 与此相同，仅 `EXPOSE` 为 8082。
...
| 服务 | 镜像 | 端口映射 | 关键配置 |
|------|------|---------|---------|
| stock-mysql-master | mysql:8.0 | 3308:3306 | binlog ROW, server-id=10, stock_db |
| stock-mysql-slave | mysql:8.0 | 3309:3306 | relay-log, read-only, server-id=11 |
| order-mysql-master | mysql:8.0 | 3310:3306 | binlog ROW, server-id=20, order_db |
| order-mysql-slave | mysql:8.0 | 3311:3306 | relay-log, read-only, server-id=21 |
| redis | redis:7-alpine | 6379:6379 | AOF持久化, 密码认证 |
| kafka | confluentinc/cp-kafka:7.5.0 | 9092:9092 | KRaft模式, 无需ZooKeeper |
| stock-service-1 | 构建自 stock-service/ | 8081:8081 | 连接 stock_db 主从 + Redis + Kafka |
| stock-service-2 | 构建自 stock-service/ | 8083:8081 | 同上 |
| order-service-1 | 构建自 order-service/ | 8082:8082 | 连接 order_db 主从 + Redis |
| order-service-2 | 构建自 order-service/ | 8084:8082 | 同上 |
| nginx | nginx:1.25-alpine | 80:80 | 按URL前缀路由到不同服务集群 |

**启动顺序保障**：通过 `depends_on` + `condition: service_healthy` 确保依赖服务健康后再启动。

### 2.4 环境变量管理

所有敏感配置通过 docker-compose.yml 的环境变量管理：

```env
MYSQL_ROOT_PASSWORD=root123
STOCK_PASSWORD=stock123
STOCK_SLAVE_PASSWORD=stock_ro123
ORDER_PASSWORD=order123
ORDER_SLAVE_PASSWORD=order_ro123
...
在 `nginx/conf.d/default.conf` 中按微服务分组配置 upstream 集群：

```nginx
upstream stock_cluster {
    server stock-service-1:8081;
    server stock-service-2:8081;
}
...
| 算法 | 配置方式 | 特点 |
|------|---------|------|
| **轮询（默认）** | `server stock-service-1:8081; server stock-service-2:8081;` | 请求均匀分配到各实例 |
| **加权轮询** | `server stock-service-1:8081 weight=3; server stock-service-2:8081 weight=1;` | 按权重分配，适合机器配置不同时 |
| **IP Hash** | `ip_hash;` | 同一IP请求固定到同一实例，适合会话保持 |
| **最少连接** | `least_conn;` | 优先分配给当前连接数最少的实例 |

### 3.3 反向代理配置

按 URL 前缀将请求路由到不同的微服务集群：

```nginx
# 库存服务 API（秒杀、商品、库存）→ stock_cluster
location ~ ^/api/(seckill|product|stock)/ {
    proxy_pass         http://stock_cluster;
