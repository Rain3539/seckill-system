...
-- ── 业务账号 ──────────────────────────────────────────────────────
CREATE USER IF NOT EXISTS 'order_user'@'%'    IDENTIFIED WITH mysql_native_password BY 'order123';
...
-- ── 主从复制专用账号 ──────────────────────────────────────────────
CREATE USER IF NOT EXISTS 'order_repl'@'%' IDENTIFIED WITH mysql_native_password BY 'OrderRepl@123';
GRANT REPLICATION SLAVE ON *.* TO 'order_repl'@'%';
