-- ── 库存服务主库初始化脚本 ──────────────────────────────────────────
-- stock_db: product, seckill_product
...
-- ── 业务账号 ──────────────────────────────────────────────────────
CREATE USER IF NOT EXISTS 'stock'@'%'    IDENTIFIED WITH mysql_native_password BY 'stock123';
GRANT ALL PRIVILEGES ON stock_db.* TO 'stock'@'%';
...
-- ── 主从复制专用账号 ──────────────────────────────────────────────
CREATE USER IF NOT EXISTS 'stock_repl'@'%' IDENTIFIED WITH mysql_native_password BY 'StockRepl@123';
GRANT REPLICATION SLAVE ON *.* TO 'stock_repl'@'%';
